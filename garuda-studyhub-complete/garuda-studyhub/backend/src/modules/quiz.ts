...
router.get('/today', requireAuth, asyncHandler(async (req, res) => {
  ...
  const attempted = await prep('SELECT id FROM quiz_attempts WHERE user_id = ? AND date = ?').get(req.user!.id, today);
...
router.post('/today/submit', requireAuth, validate(z.object({
  ...
  const today = new Date().toISOString().slice(0, 10);
  const existing = await prep('SELECT id FROM quiz_attempts WHERE user_id = ? AND date = ?').get(req.user!.id, today);
  if (existing) throw new ApiError(409, 'You already completed today\'s quiz', 'ALREADY_ATTEMPTED');

  const rows = await prep(`SELECT * FROM quiz_questions WHERE date <= ? ORDER BY date DESC LIMIT 10`).all(today) as any[];
  let score = 0;
  const answers: any[] = [];
  for (const q of rows) {
  ...
  while (true) {
    const prev = await prep('SELECT id FROM quiz_attempts WHERE user_id = ? AND date = ?').get(req.user!.id, fmt(cursor));
    if (!prev) break;
    streak += 1;
    cursor.setDate(cursor.getDate() - 1);
  }
  ...
  await prep(`INSERT INTO notifications (user_id, type, title, body) VALUES (?, 'quiz', ?, ?)`)
    .run(req.user!.id, 'Daily Quiz completed 🎯', `You scored ${score}/${rows.length} on today's quiz. Streak: ${streak} day${streak > 1 ? 's' : ''}!`);

  ok(res, { attemptId: Number(info.lastInsertRowid || (info.insertId ?? 0)), score, total: rows.length, streak }, 201);
}));
...
router.get('/result/:id', requireAuth, asyncHandler(async (req, res) => {
  ...
  const a = await prep('SELECT * FROM quiz_attempts WHERE id = ? AND user_id = ?').get(id, req.user!.id);
...
router.get('/streak', requireAuth, asyncHandler(async (req, res) => {
  const row = await prep('SELECT streak FROM quiz_attempts WHERE user_id = ? ORDER BY date DESC LIMIT 1').get(req.user!.id);
...
router.get('/questions', requireAuth, requireAdmin, asyncHandler(async (_req, res) => {
  const rows = await prep('SELECT * FROM quiz_questions ORDER BY date DESC, id DESC LIMIT 200').all();
...
router.delete('/questions/:id', requireAuth, requireAdmin, asyncHandler(async (req, res) => {
  const info = await prep('DELETE FROM quiz_questions WHERE id = ?').run(Number(req.params.id));
