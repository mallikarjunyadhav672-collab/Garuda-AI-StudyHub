...
router.put('/:id/read', asyncHandler(async (req, res) => {
  prep(`UPDATE notifications SET is_read = 1 WHERE id = ? AND user_id = ?`)
...
router.put('/read-all', asyncHandler(async (req, res) => {
  prep(`UPDATE notifications SET is_read = 1 WHERE user_id = ?`).run(req.user!.id);
...
router.post('/', requireAdmin, validate(z.object({
  ...
  const b = req.body;
  if (b.userId) {
    prep(`INSERT INTO notifications (user_id, type, title, body) VALUES (?, ?, ?, ?)`)
      .run(b.userId, b.type || 'system', b.title, b.body);
  } else {
    const users = prep('SELECT id FROM users').all() as { id: number }[];
    const stmt = prep(`INSERT INTO notifications (user_id, type, title, body) VALUES (?, ?, ?, ?)`);
    const tx = db.transaction(() => users.forEach((u) => stmt.run(u.id, b.type || 'system', b.title, b.body)));
    tx();
  }
